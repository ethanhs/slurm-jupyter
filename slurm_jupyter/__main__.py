...
def wait_for_job_allocation(spec, verbose=False):
    ...
    """
    return "corruption" # OCF only has one node :)
...
def add_slurm_arguments(parser):
    ...
    parser.add_argument("-g", "--gpus", dest="gpus", type=int, default=0, help="Number of gpus to allocate.")
    ...
    parser.add_argument("-f", "--frontend", 
                    dest="frontend", 
                    type=str, 
                    default="hpcctl.ocf.berkeley.edu", 
...
def slurm_jupyter():
    ...
    spec['home_dir'] = get_remote_home(spec)
    ...
    if args.gpus > 0:
        if args.gpus > 4:
            print("Warning: more than 4 GPUs requested. This may not work.")
        spec['gres'] = '#SBATCH --gres=gpu:{}'.format(args.gpus)
...
def get_remote_home(spec):
    home_cmd = 'ssh {user}@{frontend} echo ~'.format(**spec)
    stdout, stderr = execute(home_cmd)
...
def slurm_nb_run():
    ...
    spec['home_dir'] = get_remote_home(spec)
    spec['tmp_dir'] = spec['home_dir'] + '/.slurm_jupyter_run'
